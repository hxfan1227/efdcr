#' @import ncdf4
#' @importFrom dplyr as.tbl_cube as_tibble
...
#' @export
melt_nc <- function(ncarry, var = 'value', na.rm = F){
  ndims <- length(dim(ncarry))
  dim_names <- purrr::map(1:ndims, function(x){1:dim(ncarry)[x]})
  names(dim_names) <- paste0('Var', 1:ndims)
  dimnames(ncarry) <- dim_names
  ncarry %>% dplyr::as.tbl_cube(met_name = var) %>%
    dplyr::as_tibble() %>% data.table::setDT() -> ans
  if (na.rm) {
    ans <- na.omit(ans)
  }
  ans
